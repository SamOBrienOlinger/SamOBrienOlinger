...
The hero uses a two-column layout on wider screens and a stacked version at viewport widths of 680 px or less. Its whale photograph starts with a **1.2-second pause**, then fades in over **0.8 seconds**, once per image load. The headline is visible immediately. Both variants reserve their full dimensions throughout the reveal. Visitors with `prefers-reduced-motion: reduce` see the photograph immediately. The hero, buttons and project preview frames always use the screenshot’s dark navy palette, including when the visitor uses a light operating-system theme. GitHub controls the surrounding README background, native text and interface through each visitor’s appearance settings; a profile README cannot force those settings.
